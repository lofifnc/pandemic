...
from pandemic.model.enums import Character, OtherAction, Virus, GameState
...
def less_random_state():
    ...
    player_state._cards = {}
    state._players = {Character.SCIENTIST: PlayerState(),
                      Character.RESEARCHER: PlayerState()}
...
def test_one_card_too_many_with_event_use_event(less_random_state):
    ...
    assert len(state.get_possible_actions()) == 9
    assert all(map(lambda c: isinstance(c, ThrowCard) or isinstance(c, Event), state.get_possible_actions()))
    event = Event(EventCard.ONE_QUIET_NIGHT, Character.SCIENTIST)
